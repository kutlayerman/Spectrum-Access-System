...
def configurable_testcase(default_config_function):
    ...
    def internal_configurable_testcase(testcase):
        ...
        config_dir = os.path.join(
            harness_dir, 'testcases', 'configs', testcase.func_name)
        config_names = os.listdir(
            config_dir) if os.path.exists(config_dir) else []
...
def makePpaAndPalRecordsConsistent(ppa_record, pal_records, low_frequency,
    ...
    for index, pal_rec in enumerate(pal_records):
        ...
        pal_rec['license']['licenseDate'] = previous_year_date.strftime(
            '%Y-%m-%dT%H:%M:%SZ')
        pal_rec['license']['licenseExpiration'] = next_year_date.strftime(
            '%Y-%m-%dT%H:%M:%SZ')
    ...
    ppa_record['ppaInfo']['ppaBeginDate'] = previous_year_date.strftime(
        '%Y-%m-%dT%H:%M:%SZ')
    ppa_record['ppaInfo']['ppaExpirationDate'] = next_year_date.strftime(
        '%Y-%m-%dT%H:%M:%SZ')
    # Converting from defaultdict to dict
